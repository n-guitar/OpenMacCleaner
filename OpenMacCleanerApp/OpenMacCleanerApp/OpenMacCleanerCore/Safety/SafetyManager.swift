...
public actor SafetyManager {
    ...
    /// Delete items permanently (for Trash items)
    public func deletePermanently(_ items: [CleanupItem]) async throws -> [CleanupResult] {
        var results: [CleanupResult] = []
        
        for item in items {
            // Double check it's actually in trash to be safe
            // This is a rough check, but better than nothing
            guard item.category == .trash || item.path.path.contains("/.Trash/") else {
                results.append(CleanupResult(
                    item: item,
                    success: false,
                    error: "Item is not in Trash, cannot delete permanently safely"
                ))
                continue
            }
